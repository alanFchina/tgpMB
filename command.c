...
void     WhatVariableExp (unsigned int exp, char *st);
char     WhichAA (int x);
...
unsigned int expecting;           /* variable denoting expected token type         */
...
int				intIntegerSize;       /* size of an unsigned integer                   */
...

{
	...
	if (expecting == Expecting(PARAMETER))
		...
		if (!strcmp(parmName, "Xxxxxxxxxx"))
			...
			if (strlen(tkn) > 99)
				{
				MrBayesPrint ("%s   Charset name is too int\n", spacer);
				return (ERROR);
				}
...

{
	...
	if (expecting == Expecting(PARAMETER))
		...
		if (!strcmp(parmName, "Xxxxxxxxxx"))
			...
			if (strlen(tkn) > 99)
				{
				MrBayesPrint ("%s   Constraint name is too int\n", spacer);
				return (ERROR);
				}
...

{
	...
	int			c, i, rc, lineTerm, intestLineLength, nErrors;
	char		*s;
	FILE		*fp;
...
#	endif
	...
	/* check that intest line is not inter than CMD_STRING_LENGTH */
	if (intestLineLength >= CMD_STRING_LENGTH - 100)
		{
		MrBayesPrint ("%s   A maximum of %d characters is allowed on a single line\n", spacer, CMD_STRING_LENGTH - 100);
		MrBayesPrint ("%s   in a file. The intest line of the file %s\n", spacer, inputFileName);
...

{
	...
	if (foundFirst == NO)
		...
		for (i=1; i<NUMCOMMANDS; i++)
			{
			p = commands + i;
			if ((int)strlen(p->string) > intestDescription)
				intestDescription = (int) strlen(p->string);
			}
		...
		for (i=1; i<NUMCOMMANDS; i++)
			...
			if (p->cmdUse == IN_CMD && p->hiding == SHOW)
				...
				for (j=0; j<intestDescription - (int) strlen(p->string); j++)
				MrBayesPrint (" ");
				MrBayesPrint (" -- %s\n", p->cmdDescription);
				}
			}
...

{
	...
	if (expecting == Expecting(PARAMETER))
		...
		if (!strcmp(parmName, "Xxxxxxxxxx"))
			...
			if (strlen(tkn) > 99)
				{
				MrBayesPrint ("%s   Partition name is too int\n", spacer);
				return (ERROR);
				}
...

{
	...
	int			i, j, nameLen, start, finish, ct, intestName;
	...
	do
		...
		for (i=0; i<numTaxa; i++)
			...
			if (nameLen >= intestName)
				...
				for (j=0; j<intestName-nameLen; j++)
					MrBayesPrint (" ");
				}
			MrBayesPrint ("  ");
...

{
	...
	if (expecting == Expecting(PARAMETER))
		...
		if (!strcmp(parmName, "Xxxxxxxxxx"))
			...
			if (strlen(tkn) > 99)
				{
				MrBayesPrint ("%s   Taxset name is too int\n", spacer);
				return (ERROR);
				}
...
unsigned int Expecting (int y)

{

	unsigned int x;
	
	x = (unsigned int)pow(2.0, (MrBFlt)y);
...

{
	...
	if (!strcmp(helpTkn, "Begin"))
	    ...
	    MrBayesPrint ("   Remember that the data took a int time for you to collect. You might         \n");
	...
	/*	MrBayesPrint ("   Growthpr      -- This parameter sets the prior on the exponential growth      \n");
	    ...
	    MrBayesPrint ("   parameter aint with others, such as the tree and branch lengths. Once        \n");
