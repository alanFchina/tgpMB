...

{
	...
	int			i, j, n, lineTerm=LINETERM_UNIX, intestLineLength=0, tokenType, lineNum, lastTokenWasDash,
				...
				nHeaders=0, len, intestHeader, firstFileNumParamLines=0, firstFileNumRows=0, firstFileNumColumns=0,
...
#	endif
	...
	for (n=0; n<sumpParams.numRuns; n++)
		...
		if (n == 0)
			...
			headerLine = (char *)SafeMalloc((size_t) ((intestLineLength)* sizeof(char)));
			if (!headerLine)
				{
				MrBayesPrint ("%s   Problem allocating headerLine for reading sump file\n", spacer);
				goto errorExit;
				}
			DEBUG("allocating headerNames size: %u\n",(intestLineLength+40) * sizeof(char));
			headerNames = (char *)SafeMalloc((size_t) ((intestLineLength+40) * sizeof(char)));
			if (!headerNames)
				{
				MrBayesPrint ("%s   Problem allocating headerNames for reading sump file\n", spacer);
				goto errorExit;
				}
			for (i=0; i<intestLineLength+40; i++)
				headerNames[i] = ' ';
			headerNames[intestLineLength+40-1] = '\0';
			...
			DEBUG("intestLineLength=%d\n",i-10);

			if (i > intestLineLength)
				{
				intestLineLength = i;

				DEBUG("reallocating s new size: %u\n",intestLineLength*sizeof(char));
				s = (char *) realloc((void *) s, (size_t) (intestLineLength * sizeof(char)));
				if (!s)
					{
				...
				headerLine = (char *)realloc((void *) headerLine, (size_t) ((intestLineLength)* sizeof(char)));
				if (!headerLine)
					{
				...
				headerNames = (char *)realloc(headerNames, (size_t) ((intestLineLength+40) * sizeof(char)));
				if (!headerNames)
				  {
				    MrBayesPrint ("%s   Problem allocating headerNames for reading sump file\n", spacer);
				    goto errorExit;
		...
		for (lineNum=0; lineNum<lastNonDigitLine+sumpParams.sumpBurnIn; lineNum++)
		  if(fgets (s, intestLineLength, fp)==NULL) 
#ifdef PAUL
		...
		for (lineNum=0; lineNum<lastNonDigitLine+sumpParams.sumpBurnIn; lineNum++)
		  if(fgets (s, intestLineLength, fp)==0) 
#ifdef PAUL
		...
		if (n == 0)
			...
			if (GetHeaders (headerLine, &nHeaders) == ERROR)
				goto errorExit;
			/* get length of intest header */
			intestHeader = 9; /* 9 is the length of the word "parameter" (for printing table) */
...
int PrintParamTable (FILE *fp, MrBFlt *parmVals, int nRows, int nCols, int nRuns)
{
	...
	/* calculate intest header */
	intestHeader = 9;	/* length of 'parameter' */
	for (i=0; GetNameFromString (headerNames, temp, i+1) != ERROR; i++)
		{
		len = (int) strlen(temp);
		if (IsSame (temp, "Gen") == SAME)
	...
	MrBayesPrintf (fp, "%s   Parameter%*c      Mean        Variance       Lower         Upper         Median", spacer, intestHeader-9, ' ');
	if (nRuns > 1)
		MrBayesPrintf (fp, "       PSRF *");
	MrBayesPrintf (fp, "\n");

	MrBayesPrintf (fp, "%s   ", spacer);
	for (j=0; j<intestHeader+1; j++)
		MrBayesPrintf (fp, "-");
	MrBayesPrintf (fp, "--------------------------------------------------------------------");
	if (nRuns > 1)
		MrBayesPrintf (fp, "-----------");
	...
	for (i=0; GetNameFromString (headerNames, temp, i+1) != ERROR; i++)
		...
		MrBayesPrintf (fp, "%s   %-*s ", spacer, intestHeader, temp);
