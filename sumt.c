...
typedef struct stnode
	...
	int			scalersSet, clSpace, tiSpace;
	char			label[100];
	MrBFlt			length;
	}
	SumtNode;
...
int      AllocBits (int n);
void     AssignIntPart (int *lft, int *rht, int *p);
void     AssignTipPart (int n, int *p);
int		 BrlensVals (int treeNo, char *s, int intestLineLength, int lastTreeBlockBegin, int lastTreeBlockEnd);
...
int      FirstTaxonInPartition (int *partition, int length);
int      FreeBits (void);
...
int      PartFinder (int *p, MrBFlt bl, int *partID);
int		 PrintBrlensToFile (void);
void     PrintParts (FILE *fp, int *p, int nTaxaToShow);
int      PruneSumt (void);
...
void     ShowBits (int *p, int nBitsToShow);
...
void     ShowParts (int *p, int nTaxaToShow);
...
int	*treeBits, *treePartsFound, *taxonMask;
...

{
	...
	taxonLongsNeeded = (n / (sizeof(int)*8)) + 1;
	...
	treePartsFound = NULL;
	treePartsFound = (int *)SafeMalloc((size_t) (taxonLongsNeeded * MAX_PARTITIONS * sizeof(int)));
	if (!treePartsFound)
		{
		MrBayesPrint ("%s   Problem allocating treePartsFound (%d)\n", spacer, taxonLongsNeeded * MAX_PARTITIONS * sizeof(int));
		goto errorExit;
		}
	...
	taxonMask = NULL;
	taxonMask = (int *)SafeMalloc((size_t) (taxonLongsNeeded * sizeof(int)));
	if (!taxonMask)
		{
		MrBayesPrint ("%s   Problem allocating taxonMask (%d)\n", spacer, taxonLongsNeeded * sizeof(int));
		goto errorExit;
		}
	...
	for (i=0; i<n; i++)
		{
		offSet = 0;
		while ((i+1) > intIntegerSize*8*(offSet+1))
			offSet++;
		x = 1 << (i - offSet*intIntegerSize*8);
		y = taxonMask[offSet];
...

{
	...
	for (runNo = 0; runNo < sumtParams.numRuns; runNo++)
		...
		/* find length of intest line */
		intestLineLength = LongestLine (fp);
		intestLineLength += 10;
	
		/* allocate a string int enough to hold a line */
		if (runNo == 0)
			s = (char *) SafeMalloc (sizeof (char) * intestLineLength);
		else
			{
			free (s);
			s = (char *) SafeMalloc (sizeof (char) * intestLineLength);
		...
		while (fgets (s, intestLineLength, fp) != NULL)
			{
			sumtTokenP = &s[0];
			do
				{
...

{
	...
	int		x, *mask = NULL, *partition = NULL, *ingroupPartition = NULL, *outgroupPartition = NULL;
	MrBFlt		freq;
	char		tempName[100];
	...
	outgroupPartition = (int *) calloc (3 * taxonLongsNeeded, sizeof(int));
...
#	endif
	...
	/* find length of intest line in either file */
	intestLineLength1 = LongestLine (fp[0]);
	intestLineLength2 = LongestLine (fp[1]);
	if (intestLineLength1 > intestLineLength2)
		intestLineLength = intestLineLength1;
	else
		intestLineLength = intestLineLength2;
	MrBayesPrint ("%s   Longest line length = %d\n", spacer, intestLineLength);
	intestLineLength += 10;
	...
	s = (char *)SafeMalloc((size_t) (intestLineLength * sizeof(char)));
	if (!s)
		{
	...
	while (fgets (s, intestLineLength, fp[0]) != NULL)
		{
		sumtTokenP = &s[0];
		do
			{
	...
	foundBegin = inTreeBlock = blockErrors = inSumtComment = NO;
	lineNum = numTreesInBlock = lastTreeBlockBegin = lastTreeBlockEnd = numTreeBlocks = numTreesInLastBlock = 0;
	while (fgets (s, intestLineLength, fp[1]) != NULL)
		{
		sumtTokenP = &s[0];
		do
			{
...

{
	...
	int			i, j=0, k, n, lineTerm=0, intestLineLength=0, tokenType, foundBegin, inTreeBlock, lineNum, 
				numTreesInBlock, numTreeBlocks, lastTreeBlockBegin=0, lastTreeBlockEnd=0,
				len, lineWidth, intestName, blockErrors, inSumtComment, numExcludedTaxa, treeNo,
...
#	endif
	...
	for (treeNo = 0; treeNo < sumtParams.numTrees; treeNo++)
		...
		if (sumtBrlensDef == YES && sumtParams.printBrlensToFile == YES)
			{
			if (BrlensVals (treeNo, s, intestLineLength, lastTreeBlockBegin, lastTreeBlockEnd) == ERROR)
				goto errorExit;
			}
...

{
	...
	if (memAllocs[ALLOC_TREEPARTS] == NO)
		goto errorExit;
	treePartsFound = (int *)realloc(treePartsFound, (size_t) (taxonLongsNeeded * numPartsAllocated * sizeof(int)));
	if (!treePartsFound)
		{
		MrBayesPrint ("%s   Problem reallocating treePartsFound (%d)\n", spacer, taxonLongsNeeded * numPartsAllocated * sizeof(int));
		goto errorExit;
		}
...

{
	...
	newBits = (int *)SafeMalloc((size_t) (taxonLongsNeeded * sizeof(int)));
	if (!newBits)
		{
...

{
	...
	int			i, j, n, num, targetNode, nBits, nextConNode, isCompat, 
				...
				nInSets[5];
	int		x, *mask, *partition, *ingroupPartition, *outgroupPartition=NULL;
