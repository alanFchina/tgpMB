...
int	globalSeed;                  /* seed that is initialized at start up          */
int			nBitsInALong;                /* number of bits in a int                      */
int			readWord;					 /* should we read word next ?                    */
int	runIDSeed;                   /* seed used only for determining run ID [stamp] */
int	swapSeed;                    /* seed used only for determining which to swap  */
...
#	if defined (WIN_VERSION)
	...
	/* Allow for screen buffer 3000 lines int and 140 characters wide */
	csbi.dwSize.Y = 3000;
	csbi.dwSize.X = 140;
...
#	endif
	/*mtrace();*/
	/* calculate the size of a int - used by bit manipulation functions */
	nBitsInALong = sizeof(int) * 8;
	printf("nBitsInALong: %d\n", nBitsInALong);
	if (nBitsInALong > 32) /* Do not use more than 32 bits until we    */
		nBitsInALong = 32; /* understand how 64-bit ints are handled. */
...

{
	...
	intIntegerSize      = sizeof(int);         /* size of an integer                       */
